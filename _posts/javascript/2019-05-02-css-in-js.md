...
O _CSS-in-JS_ não é uma biblioteca especifica, ele se refere a um grupo de ideias para resolver problemas complexos do CSS, geralmente levando em conta o conceito de componentes.
...
Cada elemento terá duas classes conectadas a ele, uma é **estática** por componente, cada _styled_ terá essa classe que não liga a nenhum estilo. Em vez disso, é usada para identificar rapidamente qual elemento do DOM se referência aquele _styled-component_. Essa classe estática sempre começa com `.-sc-...`.
...
Para um elemento receber um estilo específico ele vai **depender** de uma classe, ID, de sua tag, algum tipo de dependência ele terá com o seletor CSS que o estiliza. Fora isso, também é possível que esse seletor afete outros componentes indesejavelmente, as vezes você precisa criar um seletor muito específico para atingir um elemento sem afetar outros, gerando uma dependência muito grande entre a regra CSS e o elemento do HTML.
...
Como o estilo está atrelado diretamente com o componente, o estilo só estará no browser, **só sera baixado pelo navegador caso o componente esteja sendo usado**. Se você remove o código do componente você também está **automaticamente removendo o código CSS**. Não é mais necessário percorrer todo o seu codebase procurando onde estão declaradas as regras CSS que você não precisa mais.

### Minificação

É bem comum as bibliotecas de CSS-in-JS tratar a minificação de seu CSS, chega de Gulp tasks para minificar seu código!

### Autoprefixer

Autoprefixer vem de graça, os prefixos `-webkit-`, `-moz-`, etc, são adicionados automaticamente quando necessário, um detalhe a menos para se preocupar.

## Conclusão

A forma de se escrever CSS evoluiu muito no decorrer dos anos, atualmente eu prefiro a abordagem **CSS-in-JS**.
